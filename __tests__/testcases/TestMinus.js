...
winston.configure({
  transports: [new winston.transports.File({ filename: "./__tests__/logs/Minus_logs.txt" })],
});

import keys from "../Keys.js"

async function TestMinus() {
    winston.info("test cases for sum");
  
    await keys.number1.click();
    winston.info("number 1 clicked");
  
    await keys.minus.click();
    winston.info("minus clicked");
    ...
    await keys.number0.click();
    winston.info("number 0 clicked");
